...
function draw() {
  ...
  if (bassEnergy > 180) {
    stroke("#0AFF9D"); // custom bass color (green-ish)
    strokeWeight(map(bassEnergy, 150, 255, 2, 10));
    noFill();
    beginShape();
    for (let j = 0; j < 6; j++) {
      let x = noise(noiseOffset + j * 0.1) * width;
      let y = noise(noiseOffset + j * 0.2 + 200) * height;
      vertex(x, y);
    }
    endShape();
  }
